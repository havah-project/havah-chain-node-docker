...
class ContainerTestCase(unittest.TestCase):
    ...
    compose_service_name = "havah-node"
    ...
    def _wait_until_responsive(self, check, timeout, pause, clock=timeit.default_timer, success_condition=True):
        ...
        while (now - ref) < timeout:
            ...
            count += 1
        print("")
        last_logs = self.execute(f"docker logs {self.compose_service_name} -n 40")
        ...
        try:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_point(f"height={response_json['height']}")
...
def set_os_env(home_path: str):
    os.environ['CONFIG_URL'] = _network_info_base_url
    ...
    temp_env = dict()
    temp_env['CONFIG_URL'] = os.getenv('CONFIG_URL', _network_info_base_url)
...
def set_docker_compose(home_path: str):
    ...
    os.system(f"curl -o {home_path}/goloop/config/icon_genesis.zip {_network_info_base_url}/MainNet/icon_genesis.zip")
