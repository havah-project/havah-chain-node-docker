...
def set_os_env(home_path:str):
    os.environ['CONFIG_URL'] = "https://networkinfo.havah.io/node_info"
    ...
    temp_env = dict()
    temp_env['CONFIG_URL'] = os.getenv('CONFIG_URL', 'https://networkinfo.havah.io/node_info')
...
def set_docker_compose(home_path:str):
    ...
    port = docker_compose['services']['havah-node']['environment'].get('GOLOOP_P2P_LISTEN', ':8080').split(':')[-1]
    docker_compose['services']['havah-node']['environment']['GOLOOP_P2P'] = f"{public_ip}:{port}"
    with open(f"{home_path}/goloop/docker-compose.yml", 'w') as outfile:
        yaml.dump(docker_compose, outfile)
    os.system(f"curl -o {home_path}/goloop/config/icon_genesis.zip https://networkinfo.havah.io/node_info/MainNet/icon_genesis.zip")
...
def goloop_chain_join():
    return f"docker exec -it havah-chain-node goloop chain join " \
           f"--platform icon " \
           f"--channel icon_dex " \
           ...
           f"--db_type rocksdb " \
           f"--role 0 " \
           f"--seed seed.havah.io:7100"


def goloop_chain_start():
    return f"docker exec -it havah-chain-node goloop chain start icon_dex"


def goloop_chain_leave():
    return f"docker exec -it havah-chain-node goloop chain leave icon_dex"


def goloop_chain_ls():
    return f"docker exec -it havah-chain-node goloop chain ls"
