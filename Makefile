...
GOLOOP_SRC = "https://github.com/havah-project/goloop-havah"
GOLOOP_BUILD_CMD = "goloop-havah-image"
GOLOOP_PATH = goloop-havah
...
ifeq ($(MAKECMDGOALS) , bash)
	...
	CONFIG_URL:="$(CONFIG_BASE_URL)/node_info"
	DOWNLOAD_URL:="$(CONFIG_BASE_URL)/info"
...
change_version:
		...
		@git clone $(GOLOOP_SRC) $(GOLOOP_PATH)
...
bash: make_debug_mode print_version
	docker run $(shell cat DEBUG_ARGS) -p 9000:9000 -p 7100:7100 -it -v $(PWD)/config:/goloop/config -v ${PWD}/s6:/s6-int \
		-v $(PWD)/s6/cont-finish.d:/etc/cont-finish.d -v $(PWD)/s6/cont-init.d:/etc/cont-init.d -v $(PWD)/s6/services.d:/etc/services.d\
...
s_bash: make_debug_mode print_version
	docker run $(shell cat DEBUG_ARGS) -p 9000:9000 -p 7100:7100 -it -v $(PWD)/config:/goloop/config -v ${PWD}/s6:/s6-int \
		-v ${PWD}/socket_request:/usr/lib/python3.9/site-packages/socket_request \
		-v $(PWD)/s6/cont-finish.d:/etc/cont-finish.d -v $(PWD)/s6/cont-init.d:/etc/cont-init.d -v $(PWD)/s6/services.d:/etc/services.d\
		-v $(PWD)/logs:/goloop/logs -v $(PWD)/ctx:/ctx -v $(PWD)/data:/goloop/data -e VERSION=$(TAGNAME) -v $(PWD)/src:/src --entrypoint /bin/bash \
		--name $(NAME) --cap-add SYS_TIME --rm  $(REPO_HUB)/$(NAME):$(TAGNAME)

f_bash: make_debug_mode print_version
		docker run  --rm $(shell cat DEBUG_ARGS) -p 9000:9000 -p 7100:7100 -it -v $(PWD)/config:/goloop/config \
		-v $(PWD)/s6/cont-finish.d:/etc/cont-finish.d -v $(PWD)/s6/cont-init.d:/etc/cont-init.d -v $(PWD)/s6/services.d:/etc/services.d\
		-v $(PWD)/logs:/goloop/logs -v $(PWD)/ctx:/ctx -v $(PWD)/data:/goloop/data_external -e VERSION=$(TAGNAME) -v $(PWD)/src:/src --entrypoint /bin/bash \
		--name $(NAME) --network host  $(REPO_HUB)/$(NAME):$(TAGNAME)
